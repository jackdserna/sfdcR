...
sfdc_basicQuery<- function(session, object, query) {
   ...
   job<- rforcecom.createBulkJob(
      ...
      operation = 'query',
      object = object)
   info<- rforcecom.submitBulkQuery(
      session,
      jobId = job$id,
      query = query)
   repeat{
      status<- suppressWarnings(rforcecom.checkBatchStatus(
         session,
         jobId = info$jobId,
         batchId = info$id))
   ...
   Sys.sleep(0.01)
   details<- suppressWarnings(rforcecom.getBatchDetails(
      session,
      jobId = info$jobId,
      batchId = info$id))
   result<- suppressWarnings(rforcecom.getBulkQueryResult(
      session,
      jobId = info$jobId,
      info$id,
